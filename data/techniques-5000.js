...
const qualities=['médiocre','banale','correcte','bonne','soignée','rare','remarquable','exceptionnelle','légendaire','mythique'];
const out=[]; let id=1;
for(let a=0;a<schools.length && out.length<5000;a++)for(let b=0;b<forms.length && out.length<5000;b++)for(let c=0;c<endings.length && out.length<5000;c++){
 ...
 out.push({id:`TECH-${String(id++).padStart(4,'0')}`,name:`${schools[a]} ${forms[b]} — ${endings[c]}`,rank:ranks[tier],level:tier+1,quality:qualities[quality],category:schools[a].toLowerCase(),qi:qi[i%qi.length],difficulty:5+tier*8+(i%17),comprehension:10+tier*8+(i%23),complexity:1+tier+(i%6),secret:i%17===0});
}
