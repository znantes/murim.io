...
public enum CombatAction { Strike, Guard, Evade, Grapple, Disarm, Flee }
...
public sealed class CombatSystem
{
    public CombatResult Resolve(WorldState world, Npc attacker, Npc defender, CombatAction action)
    {
        ...
        var random = new Random(HashCode.Combine(world.WorldSeed, attacker.Id, defender.Id, world.Time.Day, world.Time.MinuteOfDay, (int)action));
        var attack = OffensiveScore(attacker, action);
        var defense = DefensiveScore(defender, action);
        var hitChance = Math.Clamp(0.5 + (attack - defense) / 160.0, 0.05, 0.95);
        ...
        else if (!hit) outcome = "L'action échoue ou est évitée.";
        else
        {
            condition = BuildInjury(impact, action, world.Time.Day, random);
        ...
        defender.History.Add("Combat", defender.AgeYears, $"Subit {action} de {attacker.Identity.DisplayName} : {outcome}");
    ...
    private static PhysicalCondition BuildInjury(double impact, CombatAction action, long day, Random random)
    {
        var severity = Math.Clamp(impact / 100.0, 0.05, 1.0);
        var type = action switch
        {
            CombatAction.Grapple => severity > 0.65 ? ConditionType.Injury : ConditionType.ChronicPain,
            CombatAction.Disarm => severity > 0.75 ? ConditionType.Fracture : ConditionType.Wound,
            _ => severity > 0.82 ? ConditionType.Fracture : severity > 0.55 ? ConditionType.Wound : ConditionType.ChronicPain
        };
        return new PhysicalCondition
        {
            Type = type,
            Name = $"Blessure de combat ({action})",
            Severity = severity,
            Pain = Math.Clamp(15 + severity * 75 + random.NextDouble() * 10, 0, 100),
