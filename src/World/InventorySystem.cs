...
public sealed class InventorySystem
{
    ...
    {
        var item = new ItemDefinition { Name = name, Category = category, WeightKg = Math.Max(0, weightKg), BaseValue = Math.Max(0, baseValue), Consumable = consumable, MaxDurability = Math.Max(0, maxDurability) };
        _items[item.Id] = item;
        return item;
    }
