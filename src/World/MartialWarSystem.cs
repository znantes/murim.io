...
public sealed class MartialWarSystem
{
    ...
    public MartialWar? StartWar(WorldState world, MartialOrganization attacker, MartialOrganization defender, string cause)
    {
        ...
        var war = new MartialWar
        {
            AttackerOrganizationId = attacker.Id,
            DefenderOrganizationId = defender.Id,
            StartedDay = world.Time.Day,
            Cause = cause,
            AttackerStrength = CalculateStrength(world, attacker),
            DefenderStrength = CalculateStrength(world, defender)
        };
        _wars[war.Id] = war;
    ...
    public bool ResolveBattle(WorldState world, MartialWar war)
    {
        ...
        if (!world.MartialOrganizations.Organizations.TryGetValue(war.AttackerOrganizationId, out var attacker) ||
            !world.MartialOrganizations.Organizations.TryGetValue(war.DefenderOrganizationId, out var defender))
        {
            Conclude(war, world);
            return false;
        }
        ...
        var attackerLosses = Math.Max(1, (int)Math.Round(defenderRoll / total * Math.Max(1, attacker.MemberIds.Count) * 0.08));
        var defenderLosses = Math.Max(1, (int)Math.Round(attackerRoll / total * Math.Max(1, defender.MemberIds.Count) * 0.08));
        ...
        if (war.AttackerStrength <= 1 || war.DefenderStrength <= 1 || war.Battles >= 20)
            Conclude(war, world);
        return true;
    }
    ...
    public void Conclude(MartialWar war, WorldState world)
    {
        ...
        var conflict = world.MartialConflicts.Conflicts.Values.FirstOrDefault(c => c.Active &&
            ((c.FirstOrganizationId == war.AttackerOrganizationId && c.SecondOrganizationId == war.DefenderOrganizationId) ||
             (c.FirstOrganizationId == war.DefenderOrganizationId && c.SecondOrganizationId == war.AttackerOrganizationId)));
    ...
    {
        var members = organization.MemberIds.Select(id => world.Npcs.TryGetValue(id, out var npc) ? npc : null).Where(n => n is not null).Cast<Npc>().Where(n => n.IsAlive).ToList();
