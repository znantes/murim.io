...
public sealed class ShopStock
{
    ...
    public double TargetQuantity { get; set; } = 8;
}
...
{
    public long Day { get; init; } public int Minute { get; init; } public Guid BusinessId { get; init; } public Guid BuyerNpcId { get; init; } public Guid SellerNpcId { get; init; } public Guid ItemId { get; init; } public int Quantity { get; init; } public double UnitPrice { get; init; }
}
...
public sealed class CommerceSystem
{
    ...
    public List<BusinessAccount> Accounts { get; } = new();
    ...
    {
        var b = new CommerceBusiness { Name = name, Type = type, LocationId = locationId, OwnerNpcId = ownerNpcId, OpenEvening = evening, BuildingId = buildingId };
        _businesses[b.Id] = b; return b;
    }
    ...
    public double CurrentPrice(WorldState world, CommerceBusiness business, ShopStock stock)
    {
        ...
        var scarcity = 1.0 + Math.Max(0, stock.TargetQuantity - stock.Quantity) / Math.Max(1, stock.TargetQuantity) * .6;
        var prosperity = 1.0 + (50 - business.Prosperity) / 250;
        return Math.Max(.01, item.BaseValue * stock.PriceMultiplier * scarcity * prosperity);
    }
    public bool Buy(WorldState world, Npc buyer, CommerceBusiness business, Guid itemId, int quantity, out double total)
    {
        total = 0; if (quantity <= 0 || !business.Active || business.OwnerNpcId == buyer.Id || !business.IsOpen(world.Time.Period)) return false;
        var stock = business.Stock.FirstOrDefault(s => s.ItemId == itemId); if (stock is null || stock.Quantity < quantity || !world.Npcs.TryGetValue(business.OwnerNpcId, out var seller) || !seller.IsAlive) return false;
        var unit = CurrentPrice(world, business, stock); total = unit * quantity; if (buyer.Wealth < total || !world.Inventory.Items.TryGetValue(itemId, out var item)) return false;
        ...
        business.Capital += total; business.Reputation = Math.Min(100, business.Reputation + .05); return true;
    }
    public void AdvanceDay(WorldState world)
    {
        foreach (var b in _businesses.Values.Where(x => x.Active).ToList())
        {
            var dayTransactions = Transactions.Where(t => t.BusinessId == b.Id && t.Day == world.Time.Day).ToList();
            var revenue = dayTransactions.Sum(t => t.UnitPrice * t.Quantity);
            var payroll = b.StaffNpcIds.Select(id => world.Employment.Contracts.TryGetValue(id, out var c) ? c.WagePerHour * Math.Min(c.HoursPerDay, 8) : 0).Sum();
            var rent = Math.Max(.05, b.Type is CommerceType.MerchantHouse ? .35 : .12);
            var replenishment = b.Stock.Sum(s => Math.Max(0, s.TargetQuantity - s.Quantity) * (world.Inventory.Items.TryGetValue(s.ItemId, out var i) ? i.BaseValue * .45 : 0));
            var costs = payroll + rent + replenishment; var profit = revenue - costs; b.Capital += profit;
            b.Prosperity = Math.Clamp(b.Prosperity + profit * 2 - (b.StaffNpcIds.Count == 0 ? 1 : 0), 0, 100); b.Reputation = Math.Clamp(b.Reputation + (profit >= 0 ? .2 : -.4), 0, 100);
            Accounts.Add(new BusinessAccount { Day = world.Time.Day, Revenue = revenue, Costs = costs, Profit = profit, Capital = b.Capital });
            if (profit < 0) b.ConsecutiveLossDays++; else b.ConsecutiveLossDays = 0;
            foreach (var s in b.Stock) { var missing = Math.Max(0, (int)Math.Round(s.TargetQuantity - s.Quantity)); if (missing > 0 && b.Capital >= missing * (world.Inventory.Items.TryGetValue(s.ItemId, out var i) ? i.BaseValue * .45 : 0)) { var cost = missing * (world.Inventory.Items.TryGetValue(s.ItemId, out i) ? i.BaseValue * .45 : 0); b.Capital -= cost; s.Quantity += missing; } }
    ...
    {
        var entries = business.Stock.Where(s => s.Quantity > 0).Select(s => world.Inventory.Items.TryGetValue(s.ItemId, out var item) ? $"{item.Name} ({CurrentPrice(world, business, s):0.##}, stock {s.Quantity}/{s.TargetQuantity:0.#})" : null).Where(x => x is not null).Take(8);
        return $"{business.Name} — {(business.Active ? (business.IsOpen(world.Time.Period) ? "ouvert" : "fermé") : "faillite")} ; prospérité {business.Prosperity:0.#} ; capital {business.Capital:0.##} ; personnel {business.StaffNpcIds.Count} : {string.Join(", ", entries)}.";
    }
}
