...
public sealed class WorldState
{
    ...
    public MartialWarConsequencesSystem MartialWarConsequences { get; } = new(); public int WorldSeed { get; private set; } public Npc? PlayerNpc { get; private set; }
    ...
    {
        WorldSeed = seed; Geography.GenerateStarterRegion(seed); foreach (var location in Geography.Locations.Values) Environment.Get(location.Id);
        foreach (var location in Geography.Locations.Values)
        {
            var authority = location.Type is LocationType.Fortress or LocationType.City or LocationType.Capital ? TerritorialAuthority.Imperial : TerritorialAuthority.Independent;
            MartialTerritories.Register(location.Id, authority, null, location.Type == LocationType.Fortress ? 60 : 10, 0, this);
        }
        RegisterStarterItems(); RegisterStarterMartialArts(); var home = Geography.Locations.Values.First(l => l.Type == LocationType.Village); var random = new Random(seed);
        var origin = forcedOrigin ?? RollOrigin(random); var actualFamilyName = origin == FamilyOrigin.Imperial ? $"Maison impériale {familyName}" : familyName;
        var family = new Family { Name = actualFamilyName, Origin = origin, SocialStatus = SocialStatusFor(origin) }; AddFamily(family);
        ...
        family.FatherId = parents.Father.Id; family.MotherId = parents.Mother.Id; family.MemberIds.Add(parents.Father.Id); family.MemberIds.Add(parents.Mother.Id);
        ...
        var npc = new BirthGenerator().CreateNewborn(context, seed + 2, parents.Father, parents.Mother); AddNpc(npc); family.ChildrenIds.Add(npc.Id); family.MemberIds.Add(npc.Id); npc.JoinFamily(family.Id);
        ...
        npc.SetLocation(home.Id); parents.Father.SetLocation(home.Id); parents.Mother.SetLocation(home.Id); parents.Father.DiscoverLocation(home.Id); parents.Mother.DiscoverLocation(home.Id);
        ...
        npc.History.Add("Naissance", 0, $"Naissance au sein de la famille {family.Name}, à {home.Name}."); PlayerNpc = npc; return npc;
    }

    private void RegisterStarterItems()
    {
        Inventory.Register("Riz", ItemCategory.Food, 0.5, 1.0, true); Inventory.Register("Eau", ItemCategory.Food, 1.0, 0.5, true); Inventory.Register("Herbe médicinale", ItemCategory.Medicine, 0.1, 3.0, true); Inventory.Register("Bois", ItemCategory.Material, 1.0, 0.8); Inventory.Register("Outil simple", ItemCategory.Tool, 2.0, 12.0, false, 100);
    }

    private void RegisterStarterMartialArts()
    {
        Martial.Register("Pas du Berceau", "Tradition locale", MartialTechniqueCategory.Footwork, 12, 2, 0, 0, 4, 4); Martial.Register("Poing du Travailleur", "Tradition locale", MartialTechniqueCategory.Strike, 18, 4, 0, 5, 0, 5); Martial.Register("Garde de la Rivière", "Tradition locale", MartialTechniqueCategory.Defense, 20, 3, 0, 0, 3, 6); Martial.Register("Respiration du Matin", "Tradition locale", MartialTechniqueCategory.Internal, 35, 2, 2, 0, 0, 0);
    }

    public Npc CreateChild(int seed, Family family, Npc father, Npc mother, string culture, string region)
    {
        var context = new BirthContext { FamilyId = family.Id, FatherId = father.Id, MotherId = mother.Id, Culture = culture, SocialOrigin = family.Name, Region = region }; var child = new BirthGenerator().CreateNewborn(context, seed, father, mother);
        AddNpc(child); family.ChildrenIds.Add(child.Id); family.MemberIds.Add(child.Id); child.JoinFamily(family.Id); if (father.CurrentLocationId is Guid locationId) { child.SetLocation(locationId); child.DiscoverLocation(locationId); }
        father.History.Add("Naissance de l'enfant", father.AgeYears, $"Naissance de {child.Identity.DisplayName}."); mother.History.Add("Naissance de l'enfant", mother.AgeYears, $"Naissance de {child.Identity.DisplayName}."); return child;
    }
