...
public sealed class WorldState
{
    ...
    public InventorySystem Inventory { get; } = new(); public CommerceSystem Commerce { get; } = new(); public EmploymentSystem Employment { get; } = new(); public SurvivalSystem Survival { get; } = new(); public AgingSystem Aging { get; } = new(); public MedicineSystem Medicine { get; } = new();
    ...
    public void CreatePlayerAtBirth(int seed, string familyName)
    {
        ...
        var shop = Commerce.AddBusiness("Échoppe de Lin", CommerceType.GeneralStore, home.Id, origin.Father.Id, evening: true); Commerce.AddStock(shop, bread.Id, 12, 1.1); Commerce.AddStock(shop, water.Id, 20); Commerce.AddStock(shop, cloth.Id, 6, 1.15); Commerce.AddStock(shop, medicine.Id, 4, 1.3);
        Employment.Post(this, origin.Father, ProfessionType.Merchant, "Assistant de boutique", Math.Max(0.3, origin.Father.Profession.DailyIncome / 8.0), Math.Max(0, origin.Father.Profession.Skill), shop.Id, familyHome.Id);
    }
}
