...
public sealed class CommandInterpreter
{
    ...
    {
        ArgumentNullException.ThrowIfNull(world); input ??= string.Empty; var raw = input.Trim(); var text = Normalize(raw);
        ...
        if (Matches(text, "sors", "sortir", "sorts", "quitte le batiment", "quitte le lieu")) return Result(raw, PlayerCommandIntent.Exit);
        var enter = ExtractAfter(text, "entre dans", "entre au", "entre a", "entres dans", "entres au", "entres a");
        if (enter is not null && enter.Length > 0)
        {
            var building = ResolveBuilding(world, enter);
            return building is null ? Fail(raw, $"Je ne trouve pas de bâtiment correspondant : « {enter} ».", PlayerCommandIntent.Enter, enter) : new() { Success = true, Intent = PlayerCommandIntent.Enter, RawInput = raw, TargetText = enter, TargetBuildingId = building.Id, Feedback = $"Bâtiment : {building.Name}." };
        }
        ...
        var ask = ExtractAfter(text, "demande ton chemin a", "demande son chemin a", "demande le chemin a", "demande une direction a"); if (ask is not null) return ResolveNpcResult(world, raw, ask, PlayerCommandIntent.AskDirections);
        var follow = ExtractAfter(text, "suis", "suit"); if (follow is not null) return ResolveNpcResult(world, raw, follow, PlayerCommandIntent.Follow);
        var help = ExtractAfter(text, "aide", "aider", "aide la personne"); if (help is not null) return ResolveNpcResult(world, raw, help, PlayerCommandIntent.Help);
        var refuse = ExtractAfter(text, "refuse", "refuser", "refuse de l aider", "refuse de laider"); if (refuse is not null) return ResolveNpcResult(world, raw, refuse, PlayerCommandIntent.Refuse);
        ...
        if (travel is not null)
        {
            ...
            travel = CleanTarget(travel.Replace(" a cheval", "").Replace(" en charrette", "").Replace(" en bateau", "").Replace(" mouvement martial", "")); var location = ResolveLocation(world, travel);
        ...
        var talk = ExtractAfter(text, "parle au", "parle a", "parle avec", "parler au", "parler a", "parler avec", "discute avec", "discute a"); if (talk is not null) return ResolveNpcResult(world, raw, talk, PlayerCommandIntent.Talk);
        var examine = ExtractAfter(text, "examine", "examiner", "regarde", "regarder"); if (examine is not null && examine.Length > 0) return ResultWithTarget(raw, PlayerCommandIntent.Examine, examine);
        var train = ExtractAfter(text, "entraine toi avec", "entraine toi a", "entraine toi", "pratique", "entrainement");
        if (train is not null) { train = CleanTarget(train).Replace("a ", ""); var technique = world.Martial.Techniques.Values.FirstOrDefault(t => Normalize(t.Name) == train || Normalize(t.Name).Contains(train)); return technique is null ? Fail(raw, $"Technique inconnue : « {train} ».", PlayerCommandIntent.Train, train) : new() { Success = true, Intent = PlayerCommandIntent.Train, RawInput = raw, TargetText = train, TargetTechniqueId = technique.Id, Feedback = $"Entraînement : {technique.Name}." }; }
        var eat = ExtractAfter(text, "mange", "manger", "consomme"); if (eat is not null) { eat = CleanTarget(eat); var item = ResolveItem(world, eat, ItemCategory.Food); return item is null ? Fail(raw, $"Aucun aliment correspondant à « {eat} » dans ton inventaire.", PlayerCommandIntent.Eat, eat) : new() { Success = true, Intent = PlayerCommandIntent.Eat, RawInput = raw, TargetText = eat, TargetItemId = item.Id, Feedback = $"Consommer : {item.Name}." }; }
        var drink = ExtractAfter(text, "bois", "boire"); if (drink is not null) { drink = CleanTarget(drink); var item = ResolveItem(world, drink, null); return item is null ? Fail(raw, $"Aucune boisson correspondante à « {drink} » dans ton inventaire.", PlayerCommandIntent.Drink, drink) : new() { Success = true, Intent = PlayerCommandIntent.Drink, RawInput = raw, TargetText = drink, TargetItemId = item.Id, Feedback = $"Boire : {item.Name}." }; }
        return Fail(raw, "Commande non comprise. Essaie : observe, entre dans un bâtiment, sors, parle à quelqu'un, va à un lieu, travaille, enquête ou dors.");
    }

    private static CommandParseResult ResolveNpcResult(WorldState world, string raw, string target, PlayerCommandIntent intent) { target = CleanTarget(target); var npc = ResolveNpc(world, target); return npc is null ? Fail(raw, $"Je ne trouve pas de personne correspondante : « {target} ».", intent, target) : new() { Success = true, Intent = intent, RawInput = raw, TargetText = target, TargetNpcId = npc.Id, Feedback = $"Cible : {npc.Identity.DisplayName}." }; }
    ...
    private static CommandParseResult ResultWithTarget(string raw, PlayerCommandIntent intent, string target) => new() { Success = true, Intent = intent, RawInput = raw, TargetText = target, Feedback = "Examen." };
    ...
    private static Building? ResolveBuilding(WorldState world, string target) { var player = world.PlayerNpc; return world.Buildings.Buildings.Values.Where(b => player?.CurrentLocationId == b.LocationId).FirstOrDefault(b => Normalize(b.Name) == target || Normalize(b.Name).Contains(target)); }
