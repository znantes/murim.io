...
public enum ContextualActionKind { Talk, Travel, Examine, Enter, Exit, Work, Buy, Sell, AskDirections, Follow, Help, Refuse, PickUp, Investigate, DetectDanger, Eat, Drink, Sleep, InspectSelf, Train }
...
public sealed class ContextualActionSystem
{
    public IReadOnlyList<ContextualAction> GetAvailable(WorldState world, Npc actor)
    {
        ...
        actions.Add(new() { Kind = ContextualActionKind.InspectSelf, Label = "Voir mon état", Command = "Regarde ton état" });
        actions.Add(new() { Kind = ContextualActionKind.Eat, Label = "Manger", Command = "Mange" });
        actions.Add(new() { Kind = ContextualActionKind.Drink, Label = "Boire", Command = "Bois" });
        actions.Add(new() { Kind = ContextualActionKind.Sleep, Label = "Dormir", Command = "Dors" });
        ...
        if (actor.CurrentBuildingId is Guid currentBuildingId && world.Buildings.TryGet(currentBuildingId, out var current))
        {
            ...
            if (current.Type is BuildingType.Shop or BuildingType.Inn or BuildingType.Workshop)
            {
                actions.Add(new() { Kind = ContextualActionKind.Buy, Label = "Acheter ici", Command = "Cherche à acheter quelque chose", TargetBuildingId = current.Id });
                actions.Add(new() { Kind = ContextualActionKind.Sell, Label = "Vendre ici", Command = "Cherche à vendre quelque chose", TargetBuildingId = current.Id });
            }
            if (current.Type is BuildingType.Workshop or BuildingType.Shop or BuildingType.Inn or BuildingType.Farmhouse)
                actions.Add(new() { Kind = ContextualActionKind.Work, Label = "Travailler ici", Command = "Travaille", TargetBuildingId = current.Id });
        }
        else
        {
            foreach (var building in world.Buildings.AtLocation(locationId).Where(b => b.IsOpen(world.Time.Period)).OrderBy(b => b.Name, StringComparer.Ordinal))
            {
                ...
                if (occupants > 0)
                    actions.Add(new() { Kind = ContextualActionKind.Examine, Label = $"Observer {building.Name} ({occupants} personne(s))", Command = $"Examine {building.Name}", TargetBuildingId = building.Id });
            }
        }
        ...
        foreach (var destination in world.Geography.RoadsFrom(locationId)
                     .Where(r => actor.KnownLocationIds.Contains(r.ToLocationId))
                     .Select(r => world.Geography.Locations.TryGetValue(r.ToLocationId, out var d) ? d : null)
                     .Where(d => d is not null).Cast<Location>()
