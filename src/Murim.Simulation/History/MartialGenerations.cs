...
public sealed class MartialGenerationSystem
{
    ...
    public MartialGenerationRecord Evaluate(WorldState world, int birthYearStart, int spanYears = 12)
    {
        ...
        var innovators = cohort.Count(n => world.TechniqueVariants.Any(v => v.CreatorNpcId == n.Id));
        var deaths = cohort.Count(n => !n.IsAlive);
        var relatedEvents = world.Events.Where(e => e.ActorNpcIds.Any(id => cohort.Any(n => n.Id == id))).ToArray();
        ...
        record.NotableNpcIds.AddRange(cohort.OrderByDescending(n => Notability(n, world)).Take(12).Select(n => n.Id));
    ...
    private static double Notability(Npc npc, WorldState world)
    {
        var realm = (int)npc.Martial.Realm * 12 + (int)npc.Martial.SubRank * 2;
        var techniques = npc.Techniques.Values.Sum(t => (int)t.Stage + 1) * .7;
        var events = world.Events.Count(e => e.ActorNpcIds.Contains(npc.Id)) * .8;
        var reputation = npc.CareerStandings.Values.Sum(c => c.OrganizationTrust) * .02;
