    ...
    IReadOnlyList<TournamentArchetype> Tournaments)
{
    public static GameContent Build(int seed = 190724)
    {
        var content = new GameContent(
            TechniqueGenerator.Generate(seed + 1), MonsterGenerator.Generate(seed + 2), ProfessionCatalog.All, FactionCatalog.All,
            IngredientCatalog.All, PillGenerator.Generate(), ArtifactGenerator.Generate(seed + 3), ExtendedTournamentCatalog.All);
        ContentValidator.Validate(content); return content;
    }
}
...
public static class ContentValidator
{
    ...
    {
        TechniqueGenerator.Validate(content.Techniques); MonsterGenerator.Validate(content.Monsters);
        ...
        if (content.Pills.Count != 1_260) throw new InvalidOperationException($"Expected 1,260 pills, got {content.Pills.Count}.");
        EnsureUnique(content.Techniques.Select(x => x.Code), "technique"); EnsureUnique(content.Monsters.Select(x => x.Code), "monster");
        EnsureUnique(content.Professions.Select(x => x.Code), "profession"); EnsureUnique(content.Factions.Select(x => x.Code), "faction");
        EnsureUnique(content.Ingredients.Select(x => x.Code), "ingredient"); EnsureUnique(content.Pills.Select(x => x.Code), "pill");
        EnsureUnique(content.Artifacts.Select(x => x.Code), "artifact"); EnsureUnique(content.Tournaments.Select(x => x.Code), "tournament");
        ...
        if (!content.Tournaments.Any(t => t.Code == "dragon_phoenix_gathering")) throw new InvalidOperationException("Dragon/Phoenix gathering is missing.");
    }

    private static void EnsureUnique(IEnumerable<string> values, string label)
    {
        var all = values.ToArray();
        if (all.Distinct(StringComparer.OrdinalIgnoreCase).Count() != all.Length) throw new InvalidOperationException($"Duplicate {label} code detected.");
    }
}
...
public static class WorldIntegrity
{
    ...
    {
        var households = world.Households.Values.GroupBy(h => h.Id).Select(g => g.First()).ToArray(); world.Households.Clear();
        ...
        foreach (var npc in world.Npcs.Values)
        {
            ...
            foreach (var childId in npc.ChildIds.Where(id => !world.Npcs.ContainsKey(id)).ToArray()) npc.ChildIds.Remove(childId);
            foreach (var parentId in npc.ParentIds.Where(id => !world.Npcs.ContainsKey(id)).ToArray()) npc.ParentIds.Remove(parentId);
        }
...
public static class LivingWorldFactory
{
    ...
    {
        var content = GameContent.Build(seed); var world = WorldBootstrap.Create(seed, npcPopulation);
        WorldEnrichment.Initialize(world, content, seed); WorldIntegrity.NormalizeAndValidate(world);
        var engine = new WorldSimulationEngine(seed); return (world, content, engine);
    }

    public static LivingWorldRuntime CreateRuntime(int seed = 190724, int npcPopulation = 10_000)
    {
        var (world, content, engine) = Create(seed, npcPopulation); return new LivingWorldRuntime(world, content, engine, seed);
    }
}
