...
public static class WorldEnrichment
{
    ...
    {
        var physiology = new PhysiologyGenerator(seed + 3001); var career = new CareerProgressionSystem(); var random = new Random(seed + 3007);
        foreach (var npc in world.Npcs.Values)
        {
            ...
            SeedMartialHistory(npc, content, random, world.Clock);
        }
        RepairGenealogy(world); SeedEconomy(world); SeedEcology(world, content, random);
        var culture = new FactionCultureSystem(); foreach (var faction in world.Factions.Values) culture.GetOrCreate(world, faction.Id);
    }

    private static void SeedMartialHistory(Npc npc, GameContent content, Random random, WorldClock clock)
    {
        var combat = npc.Skills.GetValueOrDefault("combat"); if (npc.Identity.IsMonsterBorn || combat < 4) return;
        npc.Martial.Realm = combat switch { < 12 => MartialRealm.ThirdRate, < 24 => MartialRealm.SecondRate, < 39 => MartialRealm.FirstRate, < 55 => MartialRealm.Peak, < 69 => MartialRealm.TranscendentPeak, < 80 => MartialRealm.Transformation, < 90 => MartialRealm.Profound, _ => MartialRealm.LifeDeath };
        npc.Martial.SubRank = ((int)combat % 3) switch { 0 => RealmSubRank.Entry, 1 => RealmSubRank.Established, _ => RealmSubRank.Extreme };
        npc.Martial.StageProgress = Math.Clamp(combat * 1.7 % 100, 5, 98); npc.Martial.Foundation = Math.Clamp(20 + combat * .75, 10, 96); npc.Martial.Insight = Math.Clamp(combat * .65, 0, 95);
        npc.Physiology.Qi.Control = Math.Max(npc.Physiology.Qi.Control, Math.Clamp(combat * .9, 5, 96)); npc.Body.Qi = Math.Max(npc.Body.Qi, combat * .45);
        ...
        foreach (var technique in allowed)
        {
            ...
            {
                TechniqueId = technique.Id, LearnedDay = npc.Identity.BirthDay + 12 * 365L, LastPracticedDay = worldDay(clock),
                PracticePoints = Math.Clamp(20 + combat * (1.5 + random.NextDouble() * 3), 10, 760), Comprehension = Math.Clamp(combat * (.75 + random.NextDouble() * .4), 3, 96),
                ExecutionConsistency = Math.Clamp(combat * (.7 + random.NextDouble() * .45), 3, 96), PersonalAdaptation = Math.Clamp(Math.Max(0, years - 15) * random.NextDouble() * .8, 0, 65)
            };
        }
    }

    private static long worldDay(WorldClock clock) => clock.Day;
    ...
    private static void SeedEcology(WorldState world, GameContent content, Random random)
    {
        ...
        foreach (var monster in content.Monsters.Where(m => m.EvolutionStageIndex == 0).OrderBy(_ => random.Next()).Take(16))
        {
            ...
            world.Ecology[key] = new SpeciesPopulationState { SpeciesKey = monster.Code, LocationId = location.Id, Population = 8 + random.Next(60), CarryingCapacity = 25 + random.Next(120), Predator = monster.Diet is MonsterDiet.Carnivore or MonsterDiet.Parasite, PreyKeys = [] };
        }
    }

    private static string DialectFor(string? region) => region switch { "Sud" or "Sud-est" => "southern", "Nord" or "Extrême Nord" => "northern", "Ouest" or "Extrême Ouest" => "western", "Est" => "eastern", _ => "central" };
}
