...
constexpr int ID_ACTION_EDIT=1010, ID_TRAIN=1011, ID_REST=1012, ID_HOURS=1013, ID_ACTION_BUTTON=1014;
...
void button(HWND p,const wchar_t* labelText,int id){
    HWND b=CreateWindowW(L"BUTTON",labelText,WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON,0,0,120,34,p,(HMENU)(INT_PTR)id,GetModuleHandleW(nullptr),nullptr);
    SendMessageW(b,WM_SETFONT,(WPARAM)gFont,TRUE);
}
void rect(Graphics& g,float x,float y,float w,float h){
    SolidBrush fill(Color(255,15,22,31)); Pen pen(Color(255,58,72,89),1.0f);
...
void txt(Graphics& g,const std::wstring& s,float x,float y,float size,bool bold=false){
    FontFamily fam(L"Georgia"); Font f(&fam,size,bold?FontStyleBold:FontStyleRegular,UnitPixel);
    SolidBrush b(Color(255,226,190,116)); g.DrawString(s.c_str(),-1,&f,PointF(x,y),&b);
}
...
void drawPortrait(Graphics& g,float x,float y,float w,float h){
    ...
    SolidBrush cloak(Color(255,34,42,54)); g.FillEllipse(&cloak,x+w*.18f,y+h*.66f,w*.64f,h*.48f);
    SolidBrush skin(Color(255,208,161,126)); g.FillEllipse(&skin,x+w*.31f,y+h*.20f,w*.38f,h*.42f);
    SolidBrush hair(Color(255,31,24,23)); g.FillEllipse(&hair,x+w*.28f,y+h*.12f,w*.44f,h*.28f);
    g.FillRectangle(&hair,x+w*.27f,y+h*.19f,w*.10f,h*.25f); g.FillRectangle(&hair,x+w*.63f,y+h*.19f,w*.10f,h*.25f);
    SolidBrush eye(Color(255,20,20,20)); g.FillEllipse(&eye,x+w*.40f,y+h*.39f,w*.045f,h*.025f); g.FillEllipse(&eye,x+w*.555f,y+h*.39f,w*.045f,h*.025f);
    Pen line(Color(255,118,80,62),1); g.DrawLine(&line,x+w*.49f,y+h*.43f,x+w*.48f,y+h*.50f); g.DrawArc(&line,x+w*.43f,y+h*.47f,w*.15f,h*.10f,15,150);
...
void paint(HWND hwnd){
    ...
    txt(g,L"武林 · MURIM",28,13,27,true); bodyTxt(g,L"LES MILLE DESTINS · JIANGHU VIVANT · SIMULATION",31,48,11);
    bodyTxt(g,L"ANNÉE 318",W-130,15,12); bodyTxt(g,ageText(),W-130,39,12);
    const float gap=14,left=22,top=96,right=295; float pw=270, centerW=(float)W-left-pw-right-gap*2;
    if(centerW<440) centerW=440;
    rect(g,left,top,pw,H-top-20); rect(g,left+pw+gap,top,centerW,H-top-20); rect(g,left+pw+gap+centerW+gap,top,right,H-top-20);
    txt(g,L"PERSONNAGE",left+18,top+16,19,true); drawPortrait(g,left+18,top+51,pw-36,190);
    bodyTxt(g,game.started?game.name:L"Aucune vie",left+18,top+254,18); bodyTxt(g,game.started?game.title:L"—",left+18,top+282,11);
    bodyTxt(g,L"Âge · "+ageText(),left+18,top+308,12); bodyTxt(g,L"Lieu · "+game.place,left+18,top+330,12);
    txt(g,L"CORPS",left+18,top+366,13,true); bodyTxt(g,game.started?game.body:L"—",left+18,top+387,13);
    txt(g,L"ESPRIT",left+18,top+419,13,true); bodyTxt(g,game.started?game.spirit:L"—",left+18,top+440,13);
    txt(g,L"RESSOURCES",left+18,top+472,13,true); bodyTxt(g,L"Santé  "+std::to_wstring(game.health)+L" / 100",left+18,top+493,12);
    bodyTxt(g,L"Endurance  "+std::to_wstring(game.stamina)+L" / 100",left+18,top+514,12); bodyTxt(g,L"Qi  "+std::to_wstring(game.qi),left+18,top+535,12); bodyTxt(g,L"Maîtrise  "+std::to_wstring(game.martial),left+18,top+556,12);
    float cx=left+pw+gap; txt(g,L"CHRONIQUE",cx+18,top+16,19,true);
    ...
    rect(g,cx+18,top+73,centerW-36,135); txt(g,game.place,cx+34,top+91,17,true);
    ...
    txt(g,L"ACTION LIBRE",cx+18,top+238,16,true); bodyTxt(g,L"Pas de quête obligatoire. Décris ce que tu veux faire.",cx+18,top+263,11);
    txt(g,L"ENTRAÎNEMENT",cx+18,top+315,15,true); bodyTxt(g,L"Heures ce mois : "+std::to_wstring(game.trainingHours),cx+18,top+341,13);
    ...
    txt(g,L"DÉCOUVERTE",cx+18,top+430,15,true); bodyTxt(g,L"Manuels, techniques, constitutions et secrets restent cachés",cx+18,top+455,11);
    bodyTxt(g,L"jusqu'à ce que tes actions permettent de les découvrir et comprendre.",cx+18,top+474,11);
    float rx=cx+centerW+gap; txt(g,L"JIANGHU",rx+18,top+16,19,true);
    ...
    bodyTxt(g,L"95 % des personnes ont un corps ordinaire.",rx+18,top+H-top-72,11); bodyTxt(g,L"Les constitutions sont exceptionnellement rares.",rx+18,top+H-top-49,11);
    ReleaseDC(hwnd,dc);
}
void layout(HWND hwnd){
    RECT r; GetClientRect(hwnd,&r); int W=r.right,H=r.bottom;
    if(gAction) MoveWindow(gAction,310,610,std::max(300,W-650),52,TRUE); if(gHours) MoveWindow(gHours,310,560,90,32,TRUE);
    if(gLog) MoveWindow(gLog,310,665,std::max(300,W-650),std::max(100,H-690),TRUE);
    HWND b; if((b=GetDlgItem(hwnd,ID_NEW))) MoveWindow(b,W-275,18,120,36,TRUE); if((b=GetDlgItem(hwnd,ID_SAVE))) MoveWindow(b,W-145,18,120,36,TRUE);
    if((b=GetDlgItem(hwnd,ID_LOAD))) MoveWindow(b,W-275,58,120,34,TRUE); if((b=GetDlgItem(hwnd,ID_FULL))) MoveWindow(b,W-145,58,120,34,TRUE);
    if((b=GetDlgItem(hwnd,ID_TIME))) MoveWindow(b,180,560,110,32,TRUE); if((b=GetDlgItem(hwnd,ID_TRAIN))) MoveWindow(b,410,560,130,32,TRUE); if((b=GetDlgItem(hwnd,ID_REST))) MoveWindow(b,550,560,120,32,TRUE); if((b=GetDlgItem(hwnd,ID_ACTION_BUTTON))) MoveWindow(b,680,610,110,34,TRUE);
}
...
void newLife(HWND hwnd){
    ...
    logLine(L"Une nouvelle vie commence. Tu es un habitant du Jianghu."); logLine(L"Ton corps rare, s'il existe, n'est pas révélé gratuitement.");
...
LRESULT CALLBACK WndProc(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){
    ...
    case WM_CREATE:{
        ...
        gAction=CreateWindowW(L"EDIT",L"",WS_CHILD|WS_VISIBLE|WS_BORDER|ES_MULTILINE|ES_AUTOVSCROLL,0,0,400,55,hwnd,(HMENU)(INT_PTR)ID_ACTION_EDIT,GetModuleHandleW(nullptr),nullptr);SendMessageW(gAction,WM_SETFONT,(WPARAM)gSmall,TRUE);
        ...
        button(hwnd,L"Agir",ID_ACTION_BUTTON);button(hwnd,L"S'entraîner",ID_TRAIN);button(hwnd,L"Se reposer",ID_REST);
    ...
    case WM_COMMAND:{int id=LOWORD(wp);if(id==ID_NEW)newLife(hwnd);else if(id==ID_TIME)timeStep(hwnd);else if(id==ID_SAVE)saveGame(hwnd);else if(id==ID_LOAD)loadGame(hwnd);else if(id==ID_FULL)fullscreen(hwnd);else if(id==ID_ACTION_BUTTON)freeAction(hwnd);else if(id==ID_TRAIN)train(hwnd);else if(id==ID_REST)rest(hwnd);return 0;}
