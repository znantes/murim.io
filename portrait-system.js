...
(function(){
  ...
  function imageHTML(n,size){
    ...
    if(!m.url)return `<div class="${cls} murimPortraitPending" data-portrait-id="${esc(m.id)}" aria-label="Portrait IA en attente"><div class="murimPortraitPendingIcon">✦</div><span>Portrait IA</span></div>`;
  ...
  function ensureStyles(){
    ...
    const s=D.createElement('style');s.id='murimPortraitStyles';s.textContent=`
      ...
      .murimPortraitPendingIcon{font-size:24px;color:#d8af69;line-height:1}.portraitMeta{font-size:11px;color:#9ca8b5;line-height:1.4;margin-top:6px}
  ...
  function renderMainPortrait(){
    const S=W.S;if(!S||!S.player)return;const box=D.getElementById('portraitBox');if(!box)return;
    const p=S.player,m=portraitMeta(p);p.portraitId=m.id;
  ...
  function renderKnownPeople(){
    const S=W.S;if(!S)return;const box=D.getElementById('people');if(!box)return;
    ...
    box.innerHTML=people.map(n=>{const m=portraitMeta(n);n.portraitId=m.id;return `<div class="person" title="${esc(n.name)} — ${esc(n.faction||'Indépendant')}">${imageHTML(n,'small')}<div>${esc(n.name||'Inconnu')}</div><div>${Math.max(0,Number(n.age)||0)} ans</div></div>`}).join('');
    bindImageFallbacks(box);
  }
